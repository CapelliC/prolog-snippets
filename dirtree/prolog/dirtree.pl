...
:- module(dirtree,
	  [dirtree/2,                   % +Root, -Tree
	   ...
	   extension_to_embed/1,        % qualify file type to embed
	   source_target/2,             % qualify 
	   extensions_from_saved/1,     % -Extensions
	   counted_extensions/1,        % -CountedExtension
	   get_ftp_ls_output/2,
	   get_ftp_ls_output//1,
	   dirzap/1,                    % +Root remove entire subtree from Root
	   capture_attrs/3,	        % previsit visit
	   assign_path/2                % +Root, -WithPath
	  ]).
...
/** <module> Filesystem directory listing to XML.

This module provides utilities to load directories content from
